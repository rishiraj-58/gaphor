...
class SlideEditor(Gtk.Box):
    ...
    def __init__(self, slide: Slide, on_change: Callable[[], None]):
        ...
        self.title_entry.set_text(slide.title)
        ...
        self.use_region.set_active(slide.region is not None)
        ...
        capture_btn = Gtk.Button(label=gettext("Capture Current View"))
        capture_btn.connect("clicked", self._on_capture_view)
        region_box.append(capture_btn)
        ...
        if slide.region:
            self.region_x.set_value(slide.region.x)
            self.region_y.set_value(slide.region.y)
            self.region_w.set_value(slide.region.width)
            self.region_h.set_value(slide.region.height)
...
class PresentationEditor(Adw.Window):
    ...
    def _build_ui(self) -> None:
        ...
        add_btn.set_tooltip_text(gettext("Add slide from current diagram"))
    ...
    def _refresh_slides_list(self) -> None:
        ...
        for i, slide in enumerate(self.presentation.slides):
            ...
            title_label = Gtk.Label(label=f"{i + 1}. {title}")
            title_label.set_halign(Gtk.Align.START)
    ...
    def _on_add_slide(self, btn) -> None:
        """Add a slide from available diagrams."""
        from gaphor.core.modeling import Diagram
        ...
        for diagram in diagrams:
            ...
            menu_model.append(name, f"add-diagram.{diagram.id}")
