...
def test_load_pinned_presentation(element_factory, modeling_language):
    """Test that pinned state is preserved when loading a model."""
    file = buffer(
        """\
        <?xml version="1.0" encoding="utf-8"?>
    ...
    class_item = element_factory.lookup("class-item-1")
    assert class_item is not None
    assert class_item._pinned is True
